          ...
          <div className={cx("info")}>
            <div className={cx("intro")}>
              <div className={cx("price")}>
                ...
                <span className={cx("value")}>
                  {news?.type === "thuê"
                    ? `${news?.detail.price}tr/Tháng`
                    : `${news?.detail.price}/m²`}
                </span>
                {news?.type === "bán" && (
                  <span className={cx("ext")}>
                    ~{(news.detail.price * news.detail.area) / 1000}tỷ
                  </span>
                )}
              </div>
              <div className={cx("erea")}>
                ...
                <span className={cx("ext")}>
                  Mặt tiền {news?.detail.facade}m²
                </span>
              </div>
            ...
            <div className={cx("detail")}>
              ...
              <DetailEstate
                src={ereaIcon}
                title={"Diện tích"}
                value={`${news?.detail.area}m²`}
              />
              <DetailEstate
                src={homeIcon}
                title={"Mặt tiền"}
                value={`${news?.detail.facade}m²`}
              />
              <DetailEstate
                src={roomIcon}
                title={"Số phòng ngủ"}
                value={news?.detail.room}
              />
              <DetailEstate
                src={priceIcon}
                title={"Mức giá"}
                value={
                  news?.type === "thuê"
                    ? `${news?.detail.price}tr/tháng`
                    : `${news?.detail.price}tr/m²`
                }
              />
              <DetailEstate
                src={floorIcon}
                title={"Số tầng"}
                value={news?.detail.floor}
              />
              <DetailEstate
                src={toiletIcon}
                title={"Số toilet"}
                value={news?.detail.tolet}
              />
            </div>
          </div>
        </div>
        <div className={cx("sidebar")}>
          <div className={cx("contact")}>
            ...
            <div className={cx("info-contact")}>
              ...
              >
                <Button title={"Yêu cầu liên hệ lại"} primary={true} />
              </div>
              <div className={cx("popup", !isModalShow ? "hidden" : "")}>
                ...
                ></div>
                <div className={cx("modal", !isModalShow ? "hidden" : "")}>
                  <div className={cx("modal-header")}>
                    <span>Yêu cầu liên hệ lại</span>
                    <span
                      onClick={() => setIsModalShow(false)}
                      className={cx("close")}
                    >
